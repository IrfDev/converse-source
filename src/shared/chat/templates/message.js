...
export default (el, o) => {
    ...
    return html`
        ...
        >
            ...
            <a class="show-msg-author-modal" @click=${el.showUserModal}
                >${o.should_show_avatar ? renderAvatar(el.getAvatarData()) : ''}</a
            >
            <div
                class="chat-msg__content chat-msg__content--${o.sender} ${o.is_me_message
            ...
            >
                ${!o.is_me_message
                    ? html`
                          <span class="chat-msg__heading">
                              <span class="chat-msg__author"
                                  ><a class="show-msg-author-modal" @click=${el.showUserModal}>${o.username}</a></span
                              >
                              ${o.hats.map(
                                  h =>
                                      html`
                                          <span class="badge badge-secondary">${h.title} </span>
                                      `
                              )}
                              <time timestamp="${el.model.get('edited') || el.model.get('time')}" class="chat-msg__time"
                                  >${o.pretty_time}</time
                              >
                              ${o.is_encrypted
                                  ? html`
                    ...
                    : ''}
                <div
                    class="chat-msg__body chat-msg__body--${o.message_type} ${o.received
                        ? 'chat-msg__body--received'
                        : ''} ${o.is_delayed ? 'chat-msg__body--delayed' : ''}"
                >
                ...
                ${!el.model.get('hide_url_previews')
                    ? el.model.get('ogp_metadata')?.map(
                          m =>
                              html`
