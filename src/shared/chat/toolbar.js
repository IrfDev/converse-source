...
export class ChatToolbar extends CustomElement {
    ...
    render() {
        ...
        return html`
            ${this.show_toolbar
                ? html`
                      <span class="toolbar-buttons d-flex">${until(this.getButtons(), '')}</span>
                  `
                : ''}
            ${this.show_send_button
                ? html`
                      <button
                          type="submit"
                          class="btn send-button fa fa-paper-plane"
    ...
    getButtons() {
        ...
        if (message_limit) {
            buttons.push(
                html`
                    <span class="right message-limit" title="${i18n_chars_remaining}">${this.message_limit}</span>
                `
            );
        }
