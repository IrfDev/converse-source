...
export default o => {
    ...
    const avatar = html`
        <span class="mr-2">${renderAvatar(o.avatar_data)}</span>
    `;
        ...
        <div class="chatbox-title ${o.status ? '' : 'chatbox-title--no-desc'}">
            <div class="chatbox-title--row">
                ${!_converse.api.settings.get('singleton')
                    ? html`
                          <converse-controlbox-navback jid="${o.jid}"></converse-controlbox-navback>
                      `
                    : ''}
                ${o.type !== _converse.HEADLINES_TYPE
                    ? html`
                          <a class="show-msg-author-modal" @click=${o.showUserDetailsModal}>${avatar}</a>
                      `
                    : ''}
                <div class="chatbox-title__text" title="${o.jid}">
                    ${o.type !== _converse.HEADLINES_TYPE
                        ? html`
                              <a class="user show-msg-author-modal" @click=${o.showUserDetailsModal}
                                  >${o.display_name}</a
                              >
                          `
                        : o.display_name}
