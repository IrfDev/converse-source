            ...
            <li class="list-group-item">
                <form class="fingerprint-trust">
                    <div class="btn-group btn-group-toggle">
                        <label
                            class="btn btn--small ${o.device.get('trusted') !== -1
                                ? 'btn-primary active'
                                : 'btn-secondary'}"
                        >
                            <input
                                type="radio"
                                name="${o.device.get('id')}"
                                value="1"
                                ?checked=${o.device.get('trusted') !== -1}
                            />${i18n_trusted}
                        </label>
                        <label
                            class="btn btn--small ${o.device.get('trusted') !== -1
    ...
    return html`
        ...
        <ul class="list-group fingerprints">
            ...
            ${devices.length
                ...
                : html`
                      <li class="list-group-item">${i18n_no_devices}</li>
                  `}
        </ul>
    `;
};
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ${o.image
                        ? html`
                              <div class="mb-4">${avatar(Object.assign(o, avatar_data))}</div>
                          `
                        : ''}
                    ${o.fullname
                        ? html`
                              <p><label>${i18n_full_name}:</label> ${o.fullname}</p>
                          `
                        : ''}
                    <p><label>${i18n_address}:</label> <a href="xmpp:${o.jid}">${o.jid}</a></p>
                    ${o.nickname
                        ? html`
                              <p><label>${i18n_nickname}:</label> ${o.nickname}</p>
                          `
                        : ''}
                    ${o.url
                        ? html`
                              <p>
                                  <label>${i18n_url}:</label>
                    ...
                    ${o.email
                        ? html`
                              <p><label>${i18n_email}:</label> <a href="mailto:${o.email}">${o.email}</a></p>
                          `
                        : ''}
                    ${o.role
                        ? html`
                              <p><label>${i18n_role}:</label> ${o.role}</p>
                          `
                        : ''}
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-info refresh-contact">
                        <i class="fa fa-refresh"> </i>${i18n_refresh}
                    </button>
